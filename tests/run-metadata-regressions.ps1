...
$runnerCs = Join-Path $root "tests\MetadataRegressionRunner.cs"
$outDir = Join-Path $root "tests\bin"
$outExe = Join-Path $outDir "MetadataRegressionRunner.exe"
...
Write-Host "Compiling metadata regression runner..."
...
Write-Host "Running metadata regression tests..."
& $outExe
exit $LASTEXITCODE
